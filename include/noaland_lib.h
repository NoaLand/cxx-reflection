...
namespace noaland {
    ...
    template <typename T> struct is_i_dont_care : std::false_type {};

    template <> struct is_i_dont_care<i_dont_care> : std::true_type {};

    // if two type are not the same
    template <typename X, typename Y> struct is_fuzzy_type_matched {
    ...
    template <bool... R> consteval bool conjunction() {
        return (R && ...);
    }
    ...
    template <printable E> class unexpected {
    public:
    ...
    public:
        ...
        expected and_should(auto validation_func, E err)
            requires requires(decltype(validation_func) func, V v) {
                { func(v) } -> std::same_as<bool>;
            }
        {
            return std::visit(overloads{[validation_func, err](V& val) -> expected {
                                  ...
                                  [](auto&& err) -> expected { return unexpected<E>(std::move(err.what())); }},
                inner_);
        }

        expected and_then(auto operation_func)
            requires std::invocable<decltype(operation_func), V>
        {
            return std::visit(overloads{[operation_func](V& val) -> expected { return operation_func(std::move(val)); },
                                  [](auto&& err) -> expected { return unexpected<E>(std::move(err.what())); }},
                inner_);
        }

        expected or_else(auto throw_func)
            requires std::invocable<decltype(throw_func), unexpected<E>>
        {
            return std::visit(overloads{[](V& val) -> expected { return std::move(val); },
        ...
        V value_or(V&& default_val) {
            return std::visit(overloads{[](V& val) -> V { return std::move(val); },
                                  [&default_val](auto&&) -> V { return std::move(default_val); }},
                inner_);
        }
